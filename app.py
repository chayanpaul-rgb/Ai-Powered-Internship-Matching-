...
def check_user(username, password):
    conn = sqlite3.connect("fusion.db")   # use your DB name
    cur = conn.cursor()
...
def compute_matches(field_of_study, skills_csv, location):
    ...
    for internship in internships:
        ...
        # Calculate best possible score for this specific internship
        num_required_skills = len(internship["requiredSkills"])
        skills_max_score = num_required_skills * 20  # 20 points per required skill
        location_max_score = 20  # 30 points for location match
        field_max_score = 15  # 15 points for field match
        best_possible_score = skills_max_score + location_max_score + field_max_score
        
        # Skills matching (20 points per skill)
        ...
        # Field of study matching (15 points)
        if any(k in field for k in ["computer", "it", "software"]):
            score += 15
            
        # Include only if score threshold met and at least one skill matched
        if score >= 30 and len(matched_skills) > 0:
            match = dict(internship)
            # Calculate match percentage: (Actual score / Best possible score for this internship) * 100
            match_percentage = (score / best_possible_score) * 100
            match["matchScore"] = round(match_percentage, 1)  # Round to 1 decimal place
